...
outputFolder = "C:/Users/will/Documents/school/Kettering/NCPVeReMi/Data/" #Where csvs will go
...
dataset = pd.DataFrame(columns=['RecieverID','SenderID','RelX','RelY','MssgCount','dVx', 'dVy','dAx','dAy', 'AttkType']) #Stores output while program is running
    ...
    if log.endswith('.json') and not log.startswith("traceGr") and not log.startswith('._'): #Get all files without ground truth
        ...
        for _, row in data.iterrows():
            if row['type'] == 2:
                ...
                entry = [rcvID, int(row['sender']), relX, relY, count, dVx, dVy, dAx, dAy, vehicles[str(int(row['sender']))]]#Create entry for this message
